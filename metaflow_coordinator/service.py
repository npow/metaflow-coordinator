...
class FastAPIService(SessionService):
    ...
    def run(self, service_id: str, namespace: str = "metaflow-svc") -> None:
        ...
        print(
            f"[metaflow-coordinator] {service_id!r} listening on {url}"
        )

        fired = self._done.wait(timeout=self._timeout)
        if not fired:
            print(
                f"[metaflow-coordinator] {service_id!r} timed out after {self._timeout}s"
            )
...
class ProcessService(SessionService):
    ...
    def run(self, service_id: str, namespace: str = "metaflow-svc") -> None:
        ...
        self._url = url
        register_service(namespace=namespace, service_id=service_id, url=url)
        print(f"[metaflow-coordinator] process {service_id!r} at {url}")

        fired = self._done.wait(timeout=self._timeout)
        if not fired:
            print(
                f"[metaflow-coordinator] process {service_id!r} timed out after {self._timeout}s"
            )
